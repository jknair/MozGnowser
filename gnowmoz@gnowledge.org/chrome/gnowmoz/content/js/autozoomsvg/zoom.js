...
var	zmArray	   = [1,2,3,4,6,10,20,50,100,180,280,350,500,650];
...
//  with 67 loops result yelds 180 approx. You can use any figure,
//  just don't lose your magnifying glass by making it too small and unusable!
...
function init (evt) {
    ...
    */
    ...
    reset();
    //bindKEYS();
    top.keyZOOM = customzoom;
    top.keyPAN = customarrowpan;
    top.keySTOPAUTO = stopauto;
    load_svg(evt,"result.svg");
	
   
}
function Selector(a) 
{    g		      = svgdoc.createElementNS(svgNS,"g");
    g.setAttributeNS(null,"transform","translate(700 "+ (28 + 20 * a) +")");
    g.setAttributeNS(null,"id","clickText");
    g.addEventListener("mouseup",fileSelect,false);
    main.appendChild(g);
    r		      = rectangle("",0,0,0,0,160,20,"#f1f1ef","white",1,0.5,"auto");
    r.addEventListener("mouseover",function(evt) {evt.target.setAttributeNS(null,"fill","#6080ff");},false);
    r.addEventListener("mouseout",function(evt) {evt.target.setAttributeNS(null,"fill","#f1f1f0");},false);
    g.appendChild(r);
    t= text("",8,14,"'Trebuchet MS','Sans Serif'",12,"normal","normal","start","none",(files[a] + filedesc[a]),"auto","#000000");
    g.appendChild(t);
    this.selction     = r;
    this.selction.url = files[a];
   
    
}

// This is where maps and images are swapped
